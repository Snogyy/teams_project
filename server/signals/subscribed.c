...
void list_user_subscribed_teams(int client_fd, client_info_t *client)
{
    for (int i = 0; i < server.nb_teams; i++) {
        for (int j = 0; j < server.teams[i].nb_subscribers; j++) {
            if (strcmp(server.teams[i].subscribers[j], client->user_uuid) == 0) {
                ...
                generate_client_respons(client_fd, msg);
                break;
            }
        }
    }
...
void list_team_subscribers(int client_fd, char *team_uuid)
{
    int t_index = -1;
...
void signal_subscribed_list(char *argument, int client_index)
{
    ...
    if (argument != NULL && argument[0] != '\0' && argument[0] != '\r') {
        generate_client_respons(client_fd, find_reply_server(400));
        return;
    }

    generate_client_respons(client_fd, find_reply_server(240));
    list_user_subscribed_teams(client_fd, client);
    generate_client_respons(client_fd, find_reply_server(250));
}
