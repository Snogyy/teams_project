...
void handle_login_event(char *line)
{
    ...
    if (sscanf(line, "EVT LOGIN \"%36[^\"]\" \"%31[^\"]\"", user_uuid, user_name) == 2 && current_user.is_logged == true)
...
void handle_logout_event(char *line)
{
    ...
    if (sscanf(line, "EVT LOGOUT \"%36[^\"]\" \"%31[^\"]\"", user_uuid, user_name) == 2 && current_user.is_logged == true)
